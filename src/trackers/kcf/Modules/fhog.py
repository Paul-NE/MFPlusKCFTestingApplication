...
@jit
def func1(dx, dy, boundary_x, boundary_y, height, width, numChannels, NUM_SECTOR = 12):
    ...
    for j in range(1, height-1):
        for i in range(1, width-1):
            ...
            #     tx = dx[j, i] #! tx = dx[j, i, ch]
            #     ty = dy[j, i] #! ty = dy[j, i, ch]
...
@jit
def func2(dx, dy, boundary_x, boundary_y, r, alfa, nearest, w, k, height, width, sizeX, sizeY, p, stringSize, NUM_SECTOR = 12):
...
@jit
def func3(partOfNorm, mappmap, sizeX, sizeY, p, xp, pp, NUM_SECTOR = 12):
...
@jit
def func4(mappmap, p, sizeX, sizeY, pp, yp, xp, nx, ny, NUM_SECTOR = 12):
...
def getFeatureMaps(image, k, mapp, NUM_SECTOR = 12):
    ...
    sizeX = width // k
    sizeY = height // k
    px = 3 * NUM_SECTOR #! px = 3 * NUM_SECTOR
    p = px
    stringSize = sizeX * p
    ...
    mapp
    mapp['map'] = np.zeros(
        (mapp['sizeX']*mapp['sizeY']*mapp['numFeatures']), np.float32)  # readact
    ...
    r, alfa = func1(dx, dy, boundary_x, boundary_y, height,
                    width, numChannels, NUM_SECTOR)  # with @jit
    # ~0.001s
    ...
    mapp['map'] = func2(dx, dy, boundary_x, boundary_y, r, alfa, nearest,
                        w, k, height, width, sizeX, sizeY, p, stringSize, NUM_SECTOR)  # with @jit
    # ~0.001s

    return mapp


def normalizeAndTruncate(mapp, alfa, NUM_SECTOR = 12):
    sizeX = mapp['sizeX']
    sizeY = mapp['sizeY']
    ...
    newData = func3(partOfNorm, mapp['map'],
                    sizeX, sizeY, p, xp, pp, NUM_SECTOR)  # with @jit
    ###
...
def PCAFeatureMaps(mapp, NUM_SECTOR = 12):
    sizeX = mapp['sizeX']
    sizeY = mapp['sizeY']
    ...
    newData = func4(mapp['map'], p, sizeX, sizeY,
                    pp, yp, xp, nx, ny, NUM_SECTOR)  # with @jit
    ###
